...
export async function POST(req: NextRequest) {
    ...
    const prompt = `
        ...
        **INSTRUCTIONS FOR THE REPORT:**
        ...
        - **Risk-category override (highest priority).**
          *If the image transcription contains any finding that alone meets AUA criteria for a unique risk category – such as **HG T1, CIS, LVI, variant histology, or T2 (or higher)** – assign that definitive category and ignore the tumour size/number contingency below.*
        - **Fallback when decisive criteria are absent.**
          *If tumour **size and number are not reported** **and** no single finding is sufficient to fix the category, do not pick one category.
          Instead, create a **“Risk Category Possibilities”** section listing all plausible categories and give guideline-based recommendations for each. Explicitly state that exact risk cannot be determined because size/number are missing.*
